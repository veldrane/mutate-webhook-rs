use crate::config::{Config, ServerCertificate};
...
fn test_tls_loader() {
    ...
    let config = Config::default()
        .with_tls_cert(cert_path.to_str().unwrap())
        .with_tls_key(key_path.to_str().unwrap());

    let server_cert: ServerCertificate = (&config).try_into().unwrap();

    assert_eq!(server_cert.cert, CERT);
    assert_eq!(server_cert.key, KEY);
}
